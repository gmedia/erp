...
import { stockAdjustmentFormSchema, type StockAdjustmentFormData } from '@/utils/schemas';
...
const getStockAdjustmentFormDefaults = (
    ...
    return {
        ...
        warehouse_id: stockAdjustment.warehouse?.id ? String(stockAdjustment.warehouse.id) : '',
...
export const StockAdjustmentForm = memo<StockAdjustmentFormProps>(function StockAdjustmentForm({
    open,
    onOpenChange,
    stockAdjustment,
    item,
    ...
    useEffect(() => {
        const loadDetail = async () => {
            ...
            if (activeStockAdjustment.items && activeStockAdjustment.items.length > 0) {
                loadedIdRef.current = activeStockAdjustment.id;
                return;
            }

            try {
                const response = await axios.get(`/api/stock-adjustments/${activeStockAdjustment.id}`);
        ...
        loadDetail();
    }, [open, activeStockAdjustment?.id, activeStockAdjustment?.items, form]);
    ...
    return (
        <EntityForm<StockAdjustmentFormData>
            ...
            title={activeStockAdjustment ? 'Edit Stock Adjustment' : 'Add New Stock Adjustment'}
            onSubmit={onSubmit}
            isLoading={isLoading}
        ...
        >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                />
                <DatePickerField name="adjustment_date" label="Adjustment Date" />
                <Controller
                    control={form.control}
                    name="inventory_stocktake_id"
                    render={({ field }) => (
                        <div className="space-y-2">
                            <div className="text-sm font-medium leading-none">Inventory Stocktake (Optional)</div>
                            <AsyncSelect
                                value={field.value ? String(field.value) : undefined}
                                onValueChange={field.onChange}
                                ...
                                labelFn={(it) => it.stocktake_number || String(it.id)}
