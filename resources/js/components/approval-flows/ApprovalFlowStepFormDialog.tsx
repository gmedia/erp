...
const stepSchema = z.object({
    ...
    can_reject: z.string().or(z.boolean()).optional(), // handle string true/false
}).superRefine((data, ctx) => {
    if (data.approver_type === 'user' && !data.approver_user_id) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "User ID is required when Approver Type is 'Specific User'",
            path: ['approver_user_id'],
        });
    }
    if (data.approver_type === 'department_head' && !data.approver_department_id) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Department ID is required when Approver Type is 'Department Head'",
    ...
    if (data.approver_type === 'role' && !data.approver_role_id) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "Role ID is required when Approver Type is 'Specific Role'",
...
export function ApprovalFlowStepFormDialog({
    ...
    const defaultValues = useMemo(() => {
        ...
        return {
            ...
            can_reject: step.can_reject === false || step.can_reject === 'false' ? 'false' : 'true',
        };
    }, [step]);
            ...
            <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription className="sr-only">
                        {step ? 'Edit the details of this approval step.' : 'Add a new approval step to the flow.'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <Form {...form}>
                    <form onSubmit={(e) => {
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <InputField name="name" label="Step Name" placeholder="e.g. Manager Approval" />
                            <SelectField
