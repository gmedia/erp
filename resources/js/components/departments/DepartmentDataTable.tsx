...
import { Loader2, Filter, PlusCircle, Download, ChevronDown, MoreHorizontal } from 'lucide-react';
...
export function DepartmentDataTable({
    ...
    // Temporary filter state for modal (only search for departments)
    const [tempFilters, setTempFilters] = React.useState({
        search: filters?.search || '',
    });

    const columnsWithActions = React.useMemo(() => {
        return departmentColumns.map((column) => {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await axios.post('/api/departments/export', cleanFilters, {
                headers: {
            ...
            toast.error('Failed to export departments. Please try again.');
        } finally {
            setExporting(false);
        }
    };
                        ...
                        <DialogContent className="border-border bg-background text-foreground sm:max-w-[425px]">
                            <DialogHeader>
                                ...
                                <DialogDescription className="text-muted-foreground">
                                    Apply filters to find specific departments
                                </DialogDescription>
                            </DialogHeader>
                    ...
                    <TableBody>
                        {isLoading
                            ? Array.from({ length: 5 }).map((_, idx) => (
                                  <TableRow key={idx}>
                                      {Array.from({ length: departmentColumns.length }).map((_, cellIdx) => (
