            ...
            <Dialog open={open} onOpenChange={onClose}>
                <DialogContent className="max-w-4xl">
                    <DialogHeader>
                        ...
                        <DialogDescription>View GR summary and item lines</DialogDescription>
                    </DialogHeader>

                    <div className="grid grid-cols-2 gap-6 py-2">
                        <ViewField label="GR Number" value={item.gr_number} />
                        <ViewField label="PO Number" value={item.purchase_order?.po_number} />
                        <ViewField label="Supplier" value={item.purchase_order?.supplier?.name} />
                        <ViewField label="Warehouse" value={item.warehouse?.name} />
                        <ViewField
                            label="Receipt Date"
                            value={item.receipt_date ? format(new Date(item.receipt_date), 'PPP') : '-'}
                        />
                        <ViewField label="Supplier Delivery Note" value={item.supplier_delivery_note || '-'} />
                        <ViewField label="Status" value={<Badge variant="outline">{item.status}</Badge>} />
                        <ViewField label="Received By" value={item.received_by?.name || '-'} />
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b">
                                        <th className="text-left p-2">PO Item ID</th>
                                        <th className="text-left p-2">Product</th>
                                        <th className="text-left p-2">Unit</th>
                                        <th className="text-right p-2">Qty Received</th>
                                        <th className="text-right p-2">Qty Accepted</th>
                                        <th className="text-right p-2">Qty Rejected</th>
                                ...
                                <tbody>
                                    {(item.items || []).map((it) => (
                                        <tr key={it.id} className="border-b last:border-b-0">
                                            <td className="p-2">{it.purchase_order_item_id}</td>
                                            <td className="p-2">{it.product?.name || '-'}</td>
                                            <td className="p-2">{it.unit?.name || '-'}</td>
                                            <td className="p-2 text-right">{it.quantity_received}</td>
                                            <td className="p-2 text-right">{it.quantity_accepted}</td>
                                            <td className="p-2 text-right">{it.quantity_rejected}</td>
