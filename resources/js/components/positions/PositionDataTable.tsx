...
import { Download, Filter, PlusCircle, ChevronDown, MoreHorizontal } from 'lucide-react';
...
export function PositionDataTable({
    ...
    // Temporary filter state for modal (only search for positions)
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await axios.post('/api/positions/export', cleanFilters, {
                headers: {
                        ...
                        <DialogContent className="border-border bg-background text-foreground sm:max-w-[425px]">
                            <DialogHeader>
                                ...
                                <DialogDescription className="text-muted-foreground">
                                    Apply filters to find specific positions
                                </DialogDescription>
                            </DialogHeader>
                    ...
                    <TableBody>
                        {isLoading
                            ? Array.from({ length: 5 }).map((_, idx) => (
                                  <TableRow key={idx}>
                                      {Array.from({ length: positionColumns.length }).map((_, cellIdx) => (
