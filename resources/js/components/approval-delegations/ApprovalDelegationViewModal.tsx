...
export const ApprovalDelegationViewModal = memo<ApprovalDelegationViewModalProps>(
            ...
            <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
                <DialogContent className="sm:max-w-[500px]">
                    ...
                    <div className="space-y-4 py-4">
                        <ViewField label="Delegator" value={item.delegator?.name || '-'} />
                        <ViewField label="Delegate" value={item.delegate?.name || '-'} />
                        <ViewField label="Start Date" value={formatDate(item.start_date)} />
                        <ViewField label="End Date" value={formatDate(item.end_date)} />
                        <ViewField label="Approvable Type" value={formatApprovableType(item.approvable_type)} />
