...
import { inventoryStocktakeFormSchema, type InventoryStocktakeFormData } from '@/utils/schemas';
...
const getInventoryStocktakeFormDefaults = (inventoryStocktake?: InventoryStocktake | null): InventoryStocktakeFormData => {
    ...
    return {
        ...
        warehouse_id: inventoryStocktake.warehouse?.id ? String(inventoryStocktake.warehouse.id) : '',
        stocktake_date: inventoryStocktake.stocktake_date ? new Date(inventoryStocktake.stocktake_date) : new Date(),
        status: inventoryStocktake.status,
        product_category_id: inventoryStocktake.product_category?.id ? String(inventoryStocktake.product_category.id) : '',
        ...
        items: (inventoryStocktake.items || []).length
            ? (inventoryStocktake.items || []).map((item) => ({
                ...
                counted_quantity: item.counted_quantity === null || item.counted_quantity === undefined ? 0 : Number(item.counted_quantity),
                notes: item.notes || '',
            }))
            : [
                {
...
export const InventoryStocktakeForm = memo<InventoryStocktakeFormProps>(function InventoryStocktakeForm({
    open,
    onOpenChange,
    inventoryStocktake,
    item,
    ...
    useEffect(() => {
        const loadDetail = async () => {
            ...
            try {
                const response = await axios.get(`/api/inventory-stocktakes/${activeStocktake.id}`);
    ...
    return (
        <EntityForm<InventoryStocktakeFormData>
            ...
            title={activeStocktake ? 'Edit Inventory Stocktake' : 'Add New Inventory Stocktake'}
            onSubmit={onSubmit}
            isLoading={isLoading}
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <div className="space-y-2">
                                <div className="text-sm font-medium leading-none">Product Category</div>
                                <AsyncSelect
                                    value={field.value ? String(field.value) : undefined}
                                    onValueChange={field.onChange}
                    ...
                    />
                    <DatePickerField name="stocktake_date" label="Stocktake Date" />
                    <TextareaField
                        name="notes"
                        label="Notes"
                        placeholder="Notes"
