...
export const InventoryStocktakeViewModal = React.memo(
    ({ item, open, onClose }: InventoryStocktakeViewModalProps) => {
        ...
        const [detail, setDetail] = React.useState<InventoryStocktake | null>(null);
        ...
        React.useEffect(() => {
            const load = async () => {
                ...
                try {
                    const response = await axios.get(`/api/inventory-stocktakes/${item.id}`);
            ...
            <Dialog open={open} onOpenChange={onClose}>
                <DialogContent className="max-w-[95vw] sm:max-w-7xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-6 py-4">
                        <ViewField label="Stocktake Number" value={current.stocktake_number} />
                        ...
                        <ViewField label="Product Category" value={current.product_category?.name || '-'} />
                        <ViewField
                            label="Stocktake Date"
                            value={current.stocktake_date ? format(new Date(current.stocktake_date), 'PPP') : '-'}
                        />
                                    ...
                                    {(current.items || []).length === 0 ? (
                                        ...
                                        (current.items || []).map((it) => (
                                            <TableRow key={it.id}>
                                                ...
                                                <TableCell className="text-right">{it.counted_quantity ?? '-'}</TableCell>
                                                <TableCell className="text-right">{it.variance ?? '-'}</TableCell>
                                                <TableCell>
                                                    <Badge variant="outline">{it.result}</Badge>
                                                </TableCell>
