...
export const BankReconciliationWorkspace =
    memo<BankReconciliationWorkspaceProps>(function BankReconciliationWorkspace({
        bankReconciliation,
        open,
        onClose,
    }) {
        ...
        const handleAssignAccount = async (account: AccountOption) => {
            ...
            try {
                const res = await axios.put<{ data: BankReconciliationItem }>(
                ...
                toast.success(`Account assigned: ${account.code} - ${account.name}`);
                closeAssignDialog();
                await invalidate();
            } catch (err: unknown) {
                const msg =
                    rawAxios.isAxiosError(err) && err.response?.data?.message
        ...
        const handleMatch = async (jeLineId: number) => {
            ...
            try {
                const res = await axios.post<{ data: BankReconciliationItem }>(
        ...
        const handleUnmatch = async (itemId: number) => {
            ...
            try {
                ...
                setItems((prev) =>
                    prev.map((it) => (it.id === itemId ? res.data.data : it)),
                );
        ...
        const accountsAssignedCount = unmatchedItems.filter((i) => i.account_id).length;
        ...
        return (
            <>
                            ...
                            <div className="mt-3 flex flex-wrap gap-4 rounded-md border bg-muted/40 px-4 py-3 text-sm">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="font-medium text-blue-700">
                                        {accountsAssignedCount}/{unmatchedItems.length}
                                    </span>
                                </div>
                                <Separator
                                    orientation="vertical"
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span
                                        className={
                                            bankReconciliation.difference === 0
                ...
                <Dialog open={matchDialogOpen} onOpenChange={closeMatchDialog}>
                    <DialogContent className="flex max-h-[80vh] flex-col sm:max-w-3xl">
                        <DialogHeader className="shrink-0">
                            <DialogTitle>Select Journal Entry Line</DialogTitle>
                            <p className="text-sm text-muted-foreground">
                                Choose an unmatched journal entry line to link
                                to this bank item.
                            </p>
                        </DialogHeader>
                ...
                <Dialog open={assignDialogOpen} onOpenChange={closeAssignDialog}>
                    <DialogContent className="flex max-h-[70vh] flex-col sm:max-w-lg">
                        <DialogHeader className="shrink-0">
                            ...
                            <p className="text-sm text-muted-foreground">
                                Select an account to assign to this unmatched
                                bank item (e.g. bank charges, interest).
                            </p>
                        </DialogHeader>
