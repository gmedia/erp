...
type _SelectHasMeta =
    Extract<ColumnDef<Department>, { meta: unknown }> extends { meta: unknown }
        ? true
        : false;
...
const _textCol = createTextColumn<Department>({
    accessorKey: 'name',
    label: 'Name',
}) as WithAccessorKey<Department>;
type _TextAccessorKey = (typeof _textCol)['accessorKey'];
const _textAk: _TextAccessorKey = 'name';
type _TextHeader = Extract<
...
const _dateCol = createDateColumn<Department>({
...
type _DateNoSort = typeof _dateColNoSort extends { enableSorting: false }
    ? true
    : false;
...
const _emailCol = createEmailColumn<Employee>({
    accessorKey: 'email',
    label: 'Email',
...
const _phoneCol = createPhoneColumn<Employee>({
    accessorKey: 'phone',
    label: 'Phone',
}) as WithAccessorKey<Employee>;
...
const _actionsColWithCbIdCheck =
    _actionsColWithCb.id as string satisfies string;
