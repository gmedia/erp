...
> **FROZEN (2026-08-04).** Batches A–G are closed. Do **not** append new waves to this file.
> New Sonar work should start a new progress doc or an archive entry under `docs/archive/`.
> Historical detail also lives in `docs/archive/refactor-sonar-progress-archive.md`.

Dokumen ini menyimpan status batch refactor berbasis Sonar (historical tracker).

## Cara Pakai (historical)

1. ~~Update status batch di file ini setiap selesai PR.~~ — **stopped; file frozen**
2. ~~Pertahankan satu batch aktif per PR.~~ — no active batch
3. Evidence di bawah adalah snapshot terakhir (2026-06-25).
...
| G | done | 31 export classes (all FromQuery exports: asset-movements, asset-stocktakes, assets, asset-categories, asset-maintenances, asset-models, asset-locations, goods-receipts, inventory-stocktakes, purchase-orders, purchase-requests, products, stock-adjustments, stock-transfers, supplier-returns, suppliers, customers, employees, branches, departments, positions, customer-categories, supplier-categories, product-categories, units, warehouses, accounts, account-mappings, journal-entries, fiscal-years, coa-versions) | BaseExport abstract class untuk menghilangkan duplikasi constructor + query() + headings() + map() di seluruh export | Wave 1 (PR #64): 23 exports refactored, merged; Wave 2 (PR #65): 8 remaining exports + AssetExport duplicate constructor fix, 9 files, +16/−210; TLint passed, phpstan 0 errors, all tests green; full refactor: 31/31 exports extend BaseExport |
...
### OPEN issues breakdown (post-wave-17) — **superseded**

> Historical only. Superseded by **Latest Snapshot (2026-06-25)** above where Total OPEN = **0**.
> Do not treat the 83-issue table as current debt.

| Rule | Count | Severity | Effort | Notes |
|---|---|---|---|---|
| (historical aggregate) | 83 | mixed | ~165min | See git history if needed; not current |
...
- Batches A–G: ✅ done.
- **No active batch.** Remaining debt (if any) is tracked in SonarCloud, not this file.
- Final freeze snapshot health: gate OK, coverage 95.5%, dup density 0.7%, 0 bugs, 0 vulnerabilities (2026-06-25).
