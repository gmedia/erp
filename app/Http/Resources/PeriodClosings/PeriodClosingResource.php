...
class PeriodClosingResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'closed_by' => $this->whenLoaded('closedBy', fn () => ['id' => $this->resource->closed_by, 'name' => $this->resource->closedBy?->name]),
            'closed_at' => $this->resource->closed_at?->toIso8601String(),
            'reopened_by' => $this->whenLoaded('reopenedBy', fn () => ['id' => $this->resource->reopened_by, 'name' => $this->resource->reopenedBy?->name]),
