...
test('it can render stock movement report page', function () {
    ...
    actingAs($this->user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ->component('reports/stock-movement/index')
            ->has('rows.data')
        );
});
...
test('it can fetch aggregated stock movement report data', function () {
    ...
    actingAs($this->user)
        ->getJson(route('reports.stock-movement'))
        ->assertOk()
...
test('it can filter by date and dimensions', function () {
    ...
    actingAs($this->user)
        ->getJson(route('reports.stock-movement', ['product_id' => $productA->id]))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.product.id', $productA->id);

    actingAs($this->user)
        ->getJson(route('reports.stock-movement', ['warehouse_id' => $warehouseA->id]))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.warehouse.id', $warehouseA->id);

    actingAs($this->user)
        ->getJson(route('reports.stock-movement', ['branch_id' => $branchA->id]))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.warehouse.branch.id', $branchA->id);

    actingAs($this->user)
        ->getJson(route('reports.stock-movement', ['category_id' => $categoryA->id]))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.product.category.id', $categoryA->id);

    actingAs($this->user)
        ->getJson(route('reports.stock-movement', ['start_date' => '2026-03-05', 'end_date' => '2026-03-31']))
        ->assertOk()
        ->assertJsonCount(1, 'data')
...
test('it can export stock movement report', function () {
    ...
    $response = actingAs($this->user)
        ->postJson(route('reports.stock-movement.export'))
        ->assertOk()
