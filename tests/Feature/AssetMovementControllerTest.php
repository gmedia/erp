...
beforeEach(function () {
    ...
    $pMoveCreate = Permission::firstOrCreate(['name' => 'asset_movement.create', 'display_name' => 'Create Asset Movement']);
    $pMoveEdit = Permission::firstOrCreate(['name' => 'asset_movement.edit', 'display_name' => 'Edit Asset Movement']);
    $pMoveDelete = Permission::firstOrCreate(['name' => 'asset_movement.delete', 'display_name' => 'Delete Asset Movement']);
    
    $this->employee->permissions()->attach([$pAsset->id, $pEdit->id, $pMovement->id, $pMoveCreate->id, $pMoveEdit->id, $pMoveDelete->id]);
});
...
test('can list asset movements', function () {
    ...
    $response = $this->getJson(route('asset-movements.index'));
...
test('can record asset transfer', function () {
    ...
    $response = $this->postJson(route('asset-movements.store'), $data);
...
test('validates required fields for asset movements', function () {
    $response = $this->postJson(route('asset-movements.store'), []);
...
test('can list asset movements with asset_id filter', function () {
    ...
    $response = $this->getJson(route('asset-movements.index', ['asset_id' => $asset1->id]));
